...
try
{
	...
	rs6 = st6.executeQuery("select item_ID from item where names = '" + itemName + "'"); 
	rs6.next(); 
	...
	//out.println("id of cello " + ID); 
	
	//out.println("working rn " + itemName); 
	rs1 = st1.executeQuery("select names from item where item_ID = '" + ID + "'"); 
	rs2 = st2.executeQuery("select dates from item where item_ID = '" + ID + "'");
	rs3 = st3.executeQuery("select times from item where item_ID = '" + ID + "'");
	rs4 = st4.executeQuery("select initial_price from item where item_ID = '" + ID + "'");
	rs5 = st5.executeQuery("select increment_of_bids from item where item_ID = '" + ID + "'" );
